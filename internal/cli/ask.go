...
func newAskCommand(opts *options) *cobra.Command {
	...
	cmd.Flags().IntVar(&maxAgentDocumentLines, "max-agent-document-lines", agentask.DefaultMaxDocumentReadLines, "maximum lines returned by read_index_document and automatic document promotion")
	cmd.Flags().IntVar(&maxAgentDocumentTokens, "max-agent-document-tokens", agentask.DefaultMaxDocumentReadTokens, "maximum estimated tokens returned by read_index_document and automatic document promotion")
	cmd.Flags().IntVar(&maxAgentDocumentReads, "max-agent-document-reads", agentask.DefaultMaxDocumentReads, "maximum automatic document reads per agent ask run")
